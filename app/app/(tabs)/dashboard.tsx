...
type TabType = 'locked' | 'purchased' | 'in_progress' | 'completed' | 'marketplace';

export default function DashboardScreen() {
  const { user, logout } = useAuth();
  const router = useRouter();
  const params = useLocalSearchParams();
  const [activeTab, setActiveTab] = useState<TabType>((params.tab as TabType) || 'locked');
  ...
  const tabs = [
    { id: 'locked' as TabType, label: 'Locked Deals', count: lockedDeals.length },
    {
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        {activeTab === 'marketplace' ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyStateIcon}>📦</Text>
            <Text style={styles.emptyStateTitle}>No {tabs.find((t) => t.id === activeTab)?.label}</Text>
            <Text style={styles.emptyStateText}>
              {activeTab === 'locked' && 'Lock leads from the marketplace to see them here'}
              {activeTab === 'purchased' && 'Purchase locked leads to see them here'}
              {activeTab === 'in_progress' && 'Assign orders to agents to see them here'}
          ...
          <View style={styles.ordersList}>
            {filteredOrders.map((order) => (
              <OrderCard key={order.id} order={order} activeTab={activeTab} onPurchase={handlePurchase} router={router} />
            ))}
          </View>
        )}
      </ScrollView>
...
function OrderCard({ order, activeTab, onPurchase, router }: { order: Order; activeTab: TabType; onPurchase: (id: number) => void; router: any }) {
  ...
  return (
    <View style={styles.orderCard}>
      ...
      <View style={styles.orderDetails}>
        ...
        <Text style={styles.orderPrice}>{formatPrice(order.ai_estimated_price)}</Text>
      </View>
      ...
      {activeTab === 'locked' && timeRemaining !== null && (
      ...
      <View style={styles.orderActions}>
        {activeTab === 'locked' && (
          <TouchableOpacity style={styles.actionButtonPrimary} onPress={() => onPurchase(order.id)}>
        ...
        <TouchableOpacity style={styles.actionButtonSecondary} onPress={() => router.push(`/lead-detail/${order.id}`)}>
