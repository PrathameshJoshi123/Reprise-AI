...
export default function PhoneDetail() {
  ...
  if (!phoneData && passedPhone) {
    ...
    // Parse variants to extract RAM and Storage options
    const rawVariants = p.variants || [];
    const ramSet = new Map();
    const storageSet = new Map();

    if (rawVariants.length > 0) {
      rawVariants.forEach((v: any) => {
        // Format: "8/128 GB" or "128 GB"
        const parts = v.name.split("/").map((s: string) => s.trim());

        if (parts.length === 2) {
          // Has RAM / Storage
          const ram = parts[0]; // "8" or "8GB"
          const storage = parts[1]; // "128 GB"

          const ramKey = ram.toLowerCase().replace(" ", ""); // "8gb"
          const storageKey = storage.toLowerCase().replace(" ", ""); // "128gb"

          ramSet.set(ramKey, { id: ramKey, name: ram + (ram.toLowerCase().includes("gb") ? "" : "GB"), priceAdjustment: 0 });
          storageSet.set(storageKey, { id: storageKey, name: storage, priceAdjustment: 0 });

        } else if (parts.length === 1) {
          // Likely just Storage "128 GB"
          const storage = parts[0];
          const storageKey = storage.toLowerCase().replace(" ", "");
          storageSet.set(storageKey, { id: storageKey, name: storage, priceAdjustment: 0 });

          // Default RAM if missing
          ramSet.set("standard", { id: "standard", name: "Standard", priceAdjustment: 0 });
        }
      });
    } else {
      ramSet.set("standard", { id: "standard", name: "Standard", priceAdjustment: 0 });
      storageSet.set("standard", { id: "standard", name: "Standard", priceAdjustment: 0 });
    }
    ...
    phoneData = {
      ...
      ramOptions: Array.from(ramSet.values()),
  ...
  const fetchPriceFromBackend = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/calculate-price`, {
        method: "POST",
        headers: {
  ...
  const generateAIReasoning = () => {
    ...
    if (storageOption) {
      if (storageOption.priceAdjustment > 0) {
        reasons.push(
          `✓ Higher storage (${storageOption.name
    ...
    if (screenOption) {
      if (screenOption.priceAdjustment === 0) {
        ...
        reasons.push(
          `• Screen condition (${screenOption.name
                  ...
                  >
                    {phone.ramOptions.map((ram) => (
                      <div key={ram.id}>
                        ...
                        >
                          <div
                            className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedRam === ram.id
                  ...
                  >
                    {phone.storageOptions.map((storage) => (
                      <div key={storage.id}>
                        ...
                        >
                          <div
                            className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedStorage === storage.id
                              ? "border-blue-600 bg-blue-600"
                              : "border-gray-300"
                              }`}
                          >
                      ...
                      >
                        {phone.screenConditions.map((condition) => (
                          <div key={condition.id}>
                            ...
                            >
                              <div
                                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mt-0.5 ${selectedScreenCondition === condition.id
                                  ? "border-blue-600 bg-blue-600"
                                  : "border-gray-300"
                                  }`}
                              >
                    ...
                    <Button
                      onClick={() => {
                        if (!isLoggedIn) {
                          ...
                          const finalPrice = apiPrice !== null ? apiPrice : calculatePrice();
                          navigate("/checkout", {
                            state: {
                              phoneData: {
                                id: phone.id,
                                name: phone.name,
                                variant: `${phone.ramOptions.find(r => r.id === selectedRam)?.name}/${phone.storageOptions.find(s => s.id === selectedStorage)?.name}`,
                                condition: phone.screenConditions.find(s => s.id === selectedScreenCondition)?.name,
                                price: finalPrice
                              }
                            }
                          });
