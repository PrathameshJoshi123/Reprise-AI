...
export default function SellPhone() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/search-phones?q=${encodeURIComponent(searchQuery)}`);
      const data = await res.json();

      // Map backend response to frontend Phone interface
      const mapped = data.map((item: any, idx: number) => ({
        id: item.model.toLowerCase().replace(/[\s\/]+/g, '-') + '-' + idx,
        name: `${item.brand} ${item.model}`,
        brand: item.brand,
        image: "", // empty will trigger fallback
        maxPrice: item.max_price,
        variants: item.variants // Pass variants to next page
      }));
                    ...
                    <div className="relative h-[260px] md:h-[380px] rounded-2xl overflow-hidden">
                      <img
                        ...
                        onError={(e) => {
                          ...
                          ).src = `https://placehold.co/1200x400/3b82f6/ffffff?text=Slide+${index + 1
                          }`;
                        }}
                      />
